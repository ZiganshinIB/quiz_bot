...
def start(event, vk_api, db_counter):
...
def send_question(event, vk_api, quiz, db_user):
...
def check_answer(event, vk_api, quiz, db_user, db_counter):
...
def report_correct_answer(event, vk_api, quiz, db_user):
    """
...
def get_number_points(event, vk_api, db_counter):
        ...
        if event.type == VkEventType.MESSAGE_NEW and event.to_me:
            try:
                if event.text == "Начать":
                    start(event, vk_api, db_counter)
                elif event.text == "Новый вопрос":
                    send_question(event, vk_api, quiz, db_user)
                elif event.text == "Сдаться":
                    report_correct_answer(event, vk_api, quiz, db_user)
                elif event.text == "Мой счёт":
                    get_number_points(event, vk_api, db_counter)
                else:
                    check_answer(event, vk_api, quiz, db_user, db_counter)
